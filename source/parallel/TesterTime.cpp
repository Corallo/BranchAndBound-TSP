...
*/
 ...
 /*
    ...
    */
    ...
    double *solList = (double *)malloc(sizeof(double) * size);


    MPI_Allgather(&duration, 1, MPI_INT, solList, 1, MPI_INT, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);

    int min_idx = max_element(solList, solList + size) - solList;
    ...
    if (p == 0) {
        if (min_idx == p) {
            cout<<argv[1] << " Elapsed time: "<<duration<<endl;
            flag = 1; //found
        }
