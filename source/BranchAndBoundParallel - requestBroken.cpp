...
int p; 
int size;
MPI_Request requestCost;
MPI_Status statusCost;
MPI_Request requestInfo;
MPI_Status statusInfo;
MPI_Request requestProblem;
MPI_Status statusProblem;
int besttmp;
int tmpStatus;
int bestIsMine;
int* checkWorkState;
...
int ParallelTSP(int** M)
{
    ...
    if (DEEP_COM) {
        ...
        MPI_Irecv(&(M[0][0]), V * V, MPI_INT, MPI_ANY_SOURCE, 2, MPI_COMM_WORLD, &requestProblem); //Non blocking recv from all other processor to exange the best sol
        ...
        while (1) {
            ...
            if (ccc % POLLINGRATE == 0) {
                MPI_Test(&requestProblem, &flagWorkReceived, &statusProblem);
            }
            if (flagWorkReceived != 0) {//RECIVED SOMETHING
                if (statusProblem.count_lo==0){
                    cout<<"ERROR: RECIVED empty work!"<<endl;
                }
                if (DEBUGLOG) { cout << "Processor " << p << "  received some WORK TO DO from " << statusProblem.MPI_SOURCE << endl; }
                checkWorkState[p] = 0;
            ...
            if (flagInfoReceived != 0) {
                ...
                if( statusInfo.count_lo!=0){
                int source = statusInfo.MPI_SOURCE;
                if(source == p){
                    cout<<"DID I RECIEVED A MESSAGE FROM MYSELF?!?!?!??"<<endl;
                }
                cout<<"Message:" << statusInfo.count_lo<< " " << statusInfo.MPI_SOURCE <<" " << statusInfo.MPI_TAG << " "<< statusInfo.MPI_ERROR<<endl;
...
void TspStep(int** M, int step)
{
    ...
    if(COMUNICATION_SWITCH){ //constant ( #define  on top ) to enable/disable communication between processors
        ...
        if(step==0){ // first time
            MPI_Irecv(&besttmp, 1, MPI_INT, MPI_ANY_SOURCE, 0 , MPI_COMM_WORLD, &requestCost); //Non blocking recv from all other processor to exange the best sol
        ...
        if (flagBestReceived != 0) { //IF p received  something
            ...
            if (statusCost.count_lo==0){
                    cout<<"ERROR: RECIVED empty cost!"<<endl;
            }
            if (DEBUGLOG) { cout << "Processor " << p << " :   " << statusCost.MPI_SOURCE << "  sent me a new bestCost: " << besttmp << endl; }
            ...
            MPI_Irecv(&besttmp, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &requestCost); //p open a new communication channel, to wait for new sol
        }
        ...
        if (flagInfoReceived != 0) { //IF p received  something
            ...
            if (DEBUGLOG) { cout << "Processor  " << p << ":    " << statusInfo.MPI_SOURCE << "  sent me a status flag " << tmpStatus << endl; }
            ...
            MPI_Irecv(&tmpStatus, 1, MPI_INT, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, &requestInfo); //p open a new communication channel, to wait for new sol
        ...
        if(DEBUGLOG){   cout<<"Processor "<< p << " Found a better solution : "<<bestCost<<endl;}
           if(COMUNICATION_SWITCH){ //Send to all the other p
                for(int i=0;i<size;i++){
                    if(i!=p){
                        MPI_Isend(&bestCost, 1, MPI_INT, i, 0, MPI_COMM_WORLD, &requestCost);
    ...
    if((checkWorkState[neighborhood]==1 && !lock) )
    {
        ...
        if (lowerBoundB < bestCost && checkConstrain(M, V)) {
            ...
            MPI_Isend(&(M[0][0]), V * V, MPI_INT, neighborhood, 2, MPI_COMM_WORLD, &requestProblem);
...
void sendStatusInfoToAll(int status) {
    ...
    for (int i = 0; i < size; i++) {
        if (i != p) {
            ...
            MPI_Isend(&status, 1, MPI_INT, i, 1, MPI_COMM_WORLD, &requestInfo);
        }
    }
}
